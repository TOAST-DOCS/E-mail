...
**메일 도메인 관리** 탭에서 메일을 발송할 때 사용하는 발송 메일의 도메인을 관리할 수 있습니다.<br>
발송 메일의 도메인이 개인 혹은 회사에서 소유한 도메인인 경우, 제 3자에 의해 악용될 수 있는 문제점을 보완할 수 있는 기능입니다.
* 이메일 서비스를 이용하기 위해 필수로 진행해야 하는 부분은 아닙니다.
* 개인 혹은 회사에서 소유한 도메인이 아닌 경우 사용이 불가능합니다.
    * 다음/네이버/구글 등의 메일 계정을 발급해주는 메일 서비스를 제공하는 업체의 도메인은 사용할 수 없습니다.

### 메일 도메인 등록
**[메인 도메인 등록]** 버튼을 클릭하여 발송 메일에서 사용할 도메인을 등록할 수 있습니다.<br>
도메인 등록은 루트 도메인만 등록 가능합니다.

### 메일 도메인 인증
**[인증하기]** 버튼을 클릭하여 도메인에 대한 소유권 인증을 진행할 수 있습니다.<br>
도메인 등록 시 발급받은 토큰 값을 도메인의 TXT 레코드에 등록한 후 [인증하기] 버튼을 클릭하여 인증 받을 수 있습니다.<br>
* 도메인의 TXT 레코드에 "toast-domain-verification={토큰}" 값이 존재하지 않거나 혹은 일치하지 않은 경우 인증에 실패됩니다.
* 도메인에 TXT 레코드 등록 후 변경 사항이 전파되기 위해서는 다소 시간이 소요될 수 있는 점 참고 부탁드립니다.

### 메일 도메인 보호
**[보호하기]** 버튼을 클릭하여 도메인에 대한 보호를 진행할 수 있습니다.<br>
도메인 보호를 진행한 경우 아래 사항들이 적용됩니다.<br>
* 보호된 도메인은 기인증받은 프로젝트에서만 사용할 수 있습니다.
    * 동일한 도메인을 인증받은 프로젝트가 다수인 경우, 임의 한 프로젝트에서 보호 적용을 진행하여도 인증받은 프로젝트는 모두 도메인 사용이 가능합니다.
    * 예시) A, B 프로젝트의 이메일 서비스에서 "toast.com" 도메인을 인증받은 후, A 프로젝트에서 보호 적용을 진행
        * A, B 프로젝트에서만 "toast.com" 도메인을 발송 주소로 사용 가능
        * C 프로젝트에서는 "toast.com" 도메인을 인증받지 않았으므로 "toast.com" 도메인을 발송 주소로 사용 불가

### 메일 도메인 공유
**공유 > [설정]** 버튼을 클릭하여 도메인 공유가 가능합니다.<br>
주요 기능은 다음과 같습니다.<br>
* 도메인을 인증받은 프로젝트 조회
    * 특정 도메인에 대해 인증 절차를 완료한 TOAST 프로젝트 목록을 제공합니다.
* 도메인을 다른 프로젝트로 공유
    * 다수의 프로젝트에 동일한 도메인을 인증하기 위하여 인증 과정을 여러번 진행하지 않아도 됩니다.<br>
    * 한 프로젝트에서 인증을 받은 후 공유 기능을 활용하여 다른 프로젝트의 이메일 서비스 앱키 입력을 통해 도메인 공유가 가능합니다.

### 서브 도메인 등록
**메인 서브 도메인 > [설정]** 버튼을 클릭하여 서브 도메인 등록이 가능합니다.<br>
등록된 루트 도메인에 대한 서브 도메인만 등록 가능합니다.<br>
등록된 서브 도메인은 메일 발송 시 **사용자 지정 헤더(Custom header)** 기능으로 이용 가능합니다.
* 서브 도메인을 사용할 수 있는 사용자 지정 헤더(Custom header)
    * 헤더 명: X-TC-ENVELOPE-FROM
    * 헤더 값: 서브 도메인
    * 주요 기능은 다음과 같습니다.
        * 수신자의 메일 서비스에서 SPF 레코드를 검사할 도메인으로 활용
            * 예시) 받는 사람: test@toast.com, X-TC-ENVELOPE-FROM: test@cs.toast.com 으로 메일 발송 요청한 경우
                * 수신자의 메일 서비스에서 "cs.toast.com" 도메인으로 SPF 레코드 검사를 진행
        * 수신자의 메일 서비스에서 반송 메일을 보내줄 메일 주소로 활용
            * 예시) 받는 사람: test@toast.com, X-TC-ENVELOPE-FROM: test@cs.toast.com 으로 메일 발송 요청한 경우
                * 수신측 메일 서비스에서 "test@cs.toast.com" 메일 주소로 반송 메일 발송

* [사용자 지정 헤더(Custom header)](https://docs.toast.com/ko/Notification/Email/ko/Overview/#custom-header) 가이드를 참고 부탁드립니다.
