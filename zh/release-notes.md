...
* [Console] 통계 이벤트 처리 개선
    * 메일 발송간 중복으로 처리되던 통계 데이터가 중복으로 처리되지 않도록 개선되었습니다.
    * 통계 이벤트 중 "수신" 이벤트가 "수신"과 "수신 실패"로 분류됩니다.
        * "수신" 이벤트는 수신자에게 메일이 성공적으로 전달된 경우입니다.
        * "수신 실패" 이벤트는 수신자에게 메일이 전달되지 않은 경우입니다. (ex. Soft Bounce, Hard Bounce)
* [Console] 템플릿 미리보기 기능 개선
    * 템플릿 유형 중 FreeMarker 템플릿의 미리보기 기능이 개선되었습니다.
    * 템플릿 파라미터 적용시 일부 파라미터가 적용되지 않을 경우 빈 값으로 표시되고 미리보기를 확인할 수 있습니다.
