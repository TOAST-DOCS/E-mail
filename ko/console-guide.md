...
<span id="dkim"></span>
### DKIM
DKIM(Domain Keys Identified Mail)이란 발신자 및 메일 내용 등이 위변조가 되지 않았는지 수신자 측에서 검증할 수 있는 기술입니다.<br>
...
* DKIM TXT 레코드 등록 및 인증
    * **DKIM 인증** 탭을 클릭합니다.
    * 발급받은 TXT 레코드 값을 도메인의 TXT 레코드에 등록한 후 **인증** 버튼을 클릭합니다.
...
* DKIM 기능이 정상적으로 동작하는지 확인하시기 위해서는 아래와 같은 방법으로 진행하시면 됩니다.
    * 특정 발송 도메인에 대해 DKIM 인증 및 활성화
    * 특정 발송 도메인으로 이메일 발송
    * 수신받은 메일의 원본 보기
    * Authentication-Results 헤더 값에 'dkim=pass'이 포함되어 있는지 확인
