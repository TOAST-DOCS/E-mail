...
* [API] 수신 거부 링크 호출 실패 문제 개선
    * 발송된 메일 내 수신 거부 링크 클릭 시 일부 경우에서 호출이 실패하던 문제가 수정되었습니다.
    * 기존에 발송된 수신 거부 링크도 변경 후 정상적으로 동작합니다.
* [API] 비ASCII 문자가 포함된 도메인 처리 개선
    * 액센트 기호 등 비ASCII 문자가 포함된 도메인(예: `münchen.de`)으로 도메인 등록을 요청하거나 메일 발송을 요청하는 경우 오류 응답이 반환되도록 변경되었습니다.
    * RFC 1123 기준(영문자, 숫자, 하이픈, 점)을 따르지 않는 도메인 요청을 차단하여 발송 안정성이 개선됩니다.
    * 국제화 도메인을 사용해야 하는 경우, Punycode(예: `xn--mnchen-3ya.de`) 형식으로 변환하여 요청하세요.
...
* [콘솔, API] 태그(Tag) 메일 발송 기능 지원 종료
    ...
    * 콘솔에서 **태그 메일 발송 조회**, **태그 관리**, **UID 관리** 메뉴가 삭제됩니다.
    * 태그 관리, UID 관리, 태그 메일 발송 관련 API 호출 시 지원 종료 응답이 반환됩니다.
...
* [콘솔] 발송 내역 검색 결과 다운로드 시 "읽음 여부" 필드 추가
    * 일반 발송 내역 검색 결과 다운로드 결과에 "읽음 여부" 필드가 추가 되었습니다.
    * 수신자의 메일 열람 상태를 보다 쉽게 확인하고 분석할 수 있습니다.
    * 기존 검색 결과 다운로드 기능에서 제공되지 않았던 읽음 여부 정보를 통해 메일 발송 효과를 보다 정확하게 측정할 수 있습니다.
...
* [SMTP] 서브도메인 검증 로직 개선
    * SMTP 서브도메인으로 메일 발송 시 발생하던 인증 실패 문제가 수정되었습니다.
    * 서브도메인에 대한 SPF, DKIM, DMARC 인증을 모두 완료했음에도 불구하고 인증 실패로 메일이 발송되지 않던 현상이 개선되었습니다.
...
* [콘솔] 프로젝트 공유 시 SPF 레코드 및 DMARC 레코드를 재인증해야 하는 문제 수정
...
* [콘솔] 메일 요청별 조회 → 검색 결과 다운로드 요청 시 파일 내 정보 추가
...
* [콘솔] 대량 메일 발송 수신자 조회 시 common.code.null이 나오는 문제 수정
...
* [콘솔] 발송 예약을 이용해 대량 발송 시 발송 취소 버튼 추가
...
* [콘솔] 메일 발송 상세 조회 시 DSN 정보 추가
* [콘솔] 메일 발송 상세 조회 건에 대한 파일 다운로드 시 CSV 파일 내 DSN 정보 추가
...
* [콘솔] 대량 발송 수신자 엑셀/CSV 업로드 파일 크기 제한
...
* [콘솔] 태그 메일 발송 기능 추가
    ...
    * 자세한 내용은 [태그를 사용한 메일발송](./콘솔-guide/#_6)를 참고해주세요.
* [콘솔] 태그 메일 발송 내역 조회 화면 추가
...
* [콘솔] 템플릿에서 이미지 가져올 시 비정상적으로 커지는 문제 수정
    * 템플릿 본문에 이미지 태그를 넣은 경우, 콘솔에서 템플릿 선택하여 가져올 시 비정상적으로 커지는 문제가 수정되었습니다.
...
* [콘솔] 대량 메일 발송 시 오픈 통계가 정상적으로 집계되지 않는 문제
...
* [콘솔] 메일 요청별 조회 -> 메일 상세 조회 화면에서 수신 확인 여부 표시
...
* [콘솔] 메일발송 탭의 템플릿 선택 시 첨부파일 앞에 불필요한 체크박스를 제거
...
* [콘솔] 메일 요청별 조회 탭의 수신일시가 발송일시로 출력되던 현상 수정
...
* [콘솔] 메일 발송 탭에서 템플릿 적용 시 첨부파일이 제한 개수(3개)를 초과하여 발송 가능하던 현상 수정
...
* [콘솔] 대량 메일 발송 탭에서 대량 메일 발송시 메일 발송은 정상적으로 발송되나 첨부파일이 포함되지 않는 버그 수정
* [콘솔] 대량 메일 발송 탭에서 대량 메일 발송 중 오류 발생 시 발송 상태가 발송 실패로 변경되지 않는 오류 수정
* [콘솔][API] 메일 발송시 월이 변경되는 시점에 메일 발송이 실패하는 버그 수정
* [콘솔] 템플릿 관리 탭에서 존재하는 템플릿을 선택 후 신규 템플릿 생성할 경우 첨부파일 개 수가 초기화되지 않는 버그 수정
...
* [콘솔] 템플릿 관리 > 템플릿에 첨부파일 용량을 파일당 10MB로 제한하는 기능 추가 (템플릿 하나당 총 3개파일 총 30MB까지 업로드 가능)
* [콘솔] 메일발송/ 템플릿 관리 탭 내에 본문 에디터 기능 중 글머리기호, 번호매기기 기능 사용 시 에디터에서 제대로 표시되지 않던 오류 수정
* [콘솔] 메일 발송 > 메일발송 시 첨부파일 오류 후 취소 시에도 동일한 경고 발생하는 현상 수정
...
* [콘솔] 메일 발송조회 성능개선 작업 (인덱스 변경/ 반정규화 작업)
* [콘솔] 국제화처리 적용
...
* [콘솔] 수신자 리스트 파일을 2회 이상 업로드 시도 시 Progress loadbar 무한로딩되는 현상수정
