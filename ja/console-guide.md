...
    - CSV受信者ファイルのアップロード時、文字セットはUTF-8に設定する必要があります。
3. 送信したい時間がある場合は、**予約送信**欄に入力します。
4.**添付ファイルアップロード**ボタンをクリックして添付ファイルをアップロードします。
5. **タイトル及びメッセージ内容**を入力します。
...
!!! danger "注意"
    ...
    したがって、請求金額の確認や精算照合の目的では、統計メニューの件数の代わりに請求履歴をご利用ください。統計メニューは、送信傾向と結果分布を確認する用途にご活用ください。
