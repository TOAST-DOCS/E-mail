...
* [API] 受信拒否リンクの呼び出しに失敗する問題の改善
    * 送信されたメール内の受信拒否リンクをクリックした際、一部のケースで呼び出しに失敗する問題が修正されました。
    * すでに送信された受信拒否リンクも、変更後は正常に動作します。
* [API] 非ASCII文字が含まれるドメイン処理の改善
    * アクセント記号など非ASCII文字が含まれるドメイン(例：`münchen.de`)でドメイン登録をリクエストしたり、メール送信をリクエストしたりする場合に、エラーレスポンスが返却されるように変更されました。
    * RFC 1123基準(英数字、ハイフン、ドット)に従わないドメインのリクエストを遮断することで、送信の安定性が向上します。
...
#### 機能変更
* [コンソール、API] タグ(Tag)メール送信機能のサポート終了
    * タグ管理、UID管理、タグメール送信関連機能のサポートが終了します。
    * コンソールから**タグメール送信照会**、**タグ管理**、**UID管理**メニューが削除されます。
    * タグ管理、UID管理、タグメール送信関連APIの呼び出し時、サポート終了のレスポンスが返却されます。
...
* [コンソール] 送信履歴の検索結果をダウンロードする際に「既読/未読」フィールドを追加
    * 通常の送信履歴の検索結果をダウンロードしたファイルに、「既読/未読」フィールドが追加されました。
    * 受信者のメール開封状況を、より簡単に確認・分析できるようになりました。
    * 従来の検索結果ダウンロード機能では提供されていなかった開封状況の情報を通じて、メール送信の効果をより正確に測定できるようになります。

<a id="bug-fixes-2"></a>

#### バグ修正

* [SMTP] サブドメイン検証ロジックの改善
    * SMTPサブドメインを使用してメールを送信する際に発生していた認証失敗の問題が修正されました。
    * サブドメインに対するSPF、DKIM、DMARC認証を全て完了しているにもかかわらず、認証失敗によりメールが送信されなかった現象が改善されました。
...
* [コンソール]受信拒否時の通知内容の改善
    * メール受信者が受信拒否リンクをクリックした場合に表示される通知内容が改善されました。
    * 変更前：受信者のアドレスと受信拒否ボタンが表示されていました。
    * 変更後：送信者と受信拒否日時が表示されます。
    * 「送信者名<送信元メールアドレス>」形式で送信した場合、受信拒否リンクをクリックすると送信者名と送信元メールアドレスが表示されます。
        * 送信者名を入力しない場合は、送信元メールアドレスのみが表示されます。
    * この機能は5月27日以降に送信されたメールに適用され、既に送信済みのメールには適用されません。
...
* [API]ワンクリック受信拒否機能のポリシー変更
    * Gmailの送信者ガイドラインの強化に伴い、追加されたワンクリック受信拒否機能のポリシーが変更されました。
    * 変更前：受信者に送信する全てのメールに対して、ワンクリック受信拒否機能が提供されていました。
    * 変更後：受信者に送信するメールのうち、広告メールに限りワンクリック受信拒否機能が提供されます。
    * 一般メール、認証メールでワンクリック受信拒否を使用したい場合は、`customHeaders`に
      `List-Unsubscribe-Post: List-Unsubscribe=One-Click`を追加して使用できます。
        * その際、ワンクリック受信拒否のURLはユーザーが直接作成する必要があり、サービス側で内部的に自動生成されたリンクは提供されません。
...
* [コンソール]大量送信の統計処理基準の変更及びエラーの改善
    * 大量送信時の統計処理基準が変更されました。
        * ユーザーがアップロードしたファイルが処理される時点で、リクエストイベントが発生するようになりました。
    * 一部重複して処理されていた統計イベントを改善しました。
...
* [コンソール] Freemarkerテンプレートプレビュー機能を改善
...
* [コンソール]テンプレート管理機能追加 - カテゴリー/テンプレート修正
...
* [コンソール]大量メール受信者ファイルアップロード制限変更案内
...
* [コンソール]テンプレート管理機能追加 - カテゴリー/テンプレートの修正
    * テンプレート及びカテゴリー修正機能が追加され、選択した項目を修正できます。
...
* [コンソール]メール送信照会時にSMTPレスポンスコードフィールドを追加
...
* [コンソール]プロジェクト共有時にSPFレコードとDMARCレコードを再認証しなければならない問題を修正
...
* [コンソール]メールリクエスト別照会→検索結果のダウンロード要求時にファイル内の情報を追加しました。
...
* [コンソール]メールテンプレート登録時、一部CSSのクラッシュを修正
...
* [コンソール]大量メール送信受信者照会時にcommon.code.nullが発生する問題を修正
...
* [コンソール]送信予約を利用して大量送信を行う時に送信キャンセルボタンを追加
...
* [コンソール]メール送信詳細の照会時、DSN情報を追加
* [コンソール]メール送信詳細照会結果のファイルダウンロード時、CSVファイル内にDSN情報を追加
...
* [コンソール] **大量メール送信照会** タブ内ダウンロード機能を追加
...
* [コンソール]大量送信受信者Excel/CSVアップロードファイルサイズ制限
...
* [コンソール]テンプレートから画像のインポートを行うと異常に大きくなる問題を修正
...
* [コンソール]大量メール送信時、オープン統計が正常に集計されない問題
...
* [コンソール] <b>受信者名<メールアドレス></b> 形式で送信が可能
...
* [コンソール]メールリクエスト別照会 -> メール詳細照会画面で受信確認有無を表示
...
* [コンソール]メール送信タブのテンプレート選択時、添付ファイル前の不要なチェックボックスを削除
...
* [コンソール]メールリクエスト別照会タブの受信日時が送信日時として出力されていた現象を修正
...
* [コンソール]メール送信タブでテンプレート適用時、添付ファイルが制限数(3個)を超えて送信できていた現象を修正
...
* [コンソール]大量メール送信タブから大量メール送信時、メール送信は正常に行われるが添付ファイルが含まれないバグを修正
* [コンソール]大量メール送信タブで大量メール送信中にエラー発生時、送信ステータスが送信失敗に変更されないエラーを修正
* [コンソール][API]メール送信時、月が変更された時点でメール送信が失敗するバグを修正
* [コンソール]テンプレート管理タブで存在するテンプレートを選択した後に新規テンプレートを作成する場合、添付ファイルの数が初期化されないバグを修正
...
* [コンソール]ファイルアップロードによる大量メール送信機能を改善
...
* [コンソール]テンプレート管理 > テンプレートに添付ファイル容量をファイルあたり10MBに制限する機能を追加(
  テンプレート1つあたり合計3個ファイル、合計30MBまでアップロード可能)
* [コンソール]メール送信/ テンプレート管理タブ内に本文エディタ機能のうち、箇条書き、番号付け機能使用時、エディタで正常に表示されないエラーを修正
* [コンソール]メール送信 > メール送信時、添付ファイルエラー後キャンセル時にも同じ警告が発生する現象を修正
...
* [コンソール]メール送信照会性能改善作業(インデックス変更/ 非正規化作業)
* [コンソール]国際化処理適用
...
* [コンソール]受信者リストファイルを2回以上アップロードしようとしたとき、Progress loadbarが無限ローディングする現象を修正
